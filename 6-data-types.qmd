...
**You want to transform set all values in `ethics_1` coded as "3" to "NA". Type in the correct command!**
...
Create a tibble that contains the following data (N = 2,000 observations):

-   The variable `fruit`, a character vector that includes the word `banana` and `apple` in repetition (i.e., `banana`, `fruit`, `banana`, `fruit`, etc.)
-   The variable `rating`, a numeric vector including ratings from 1 to 5, in a randomized order. (**Tip**: You need to figure out how you can randomize numbers!)
-   The variable `label_rating`, a factor including levels for each rating (matching the number in `rating`): `1 = very good`, `2 = good`, `3 = average`, `4 = bad`, and `5 = very bad`.
