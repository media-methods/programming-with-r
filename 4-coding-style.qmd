...
In this course, we will primarily work with the tidyverse. In practice, most R users learn to read both styles but typically choose one main style within a project to keep code consistent.
...
In this course, we follow the [tidyverse style guide](https://style.tidyverse.org).
...
**Which of the following lines of code follow any of the coding style mentioned before consistently?**
```{r, quiz-4-2,eval = FALSE, echo = TRUE}
//| echo: false

MC_coding_styles_2 = [
    ["survey_data <- read.csv(\"data.csv\")", "True"],
    ["surveyData <- clean_data(survey_data)", "False"],
    ["survey.data <- analyze_data(survey.data)", "True"],
    ["survey_data <- transformData(survey_data)", "False"],
    ["surveydata <- summarize_results(surveydata)", "True"]
]

viewof answers_coding_styles_2 = quizInput({
  questions: MC_coding_styles_2,
  options: ["Consistent style", "Not consistent style"]
})
```
:::